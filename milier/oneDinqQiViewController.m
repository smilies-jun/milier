...
@interface oneDinqQiViewController (){
    NSMutableArray *dataArray;
}

@end

@implementation oneDinqQiViewController
...
-(void)getNetworkData:(BOOL)isRefresh
{
    ...
    if (isRefresh) {
        url = [NSString stringWithFormat:@"%@?page=1&rows=20&userId=%@&productCategoryId=0&state=0",PRODUCTO_RDERS_URL,userID];
        
    }else{
        url = [NSString stringWithFormat:@"%@?page=%d&rows=20&userId=%@&productCategoryId=0&state=0",PRODUCTO_RDERS_URL,page,userID];
