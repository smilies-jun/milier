//
//  SYNetWorkingUpLoad.m
//  shiyi
//
//  Created by 于君 on 15/6/5.
//  Copyright (c) 2015年 于君. All rights reserved.
//

#import "SYNetWorkingUpLoad.h"

@implementation SYNetWorkingUpLoad
+(instancetype)networkUpload
{
    SYNetWorkingUpLoad *network= [[SYNetWorkingUpLoad alloc]init];
    return network;
}
...
-(void)updataVideoWithUrl:(NSString *)url withParams:(NSDictionary *)params withImage:(NSArray *)arrImage withCompressionQuality:(CGFloat)compressionQuality
{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
        ...
        NSString *myCodeStr =[NSString stringWithFormat:@"%@",[dic objectForKey:@"code"]] ;
        if ([myCodeStr intValue] == 200) {
            ...
            if ([self.delegate respondsToSelector:@selector(requestErrorFromServer:)]) {
                [self.delegate requestErrorFromServer:@"请求超时"];
            }
        }
