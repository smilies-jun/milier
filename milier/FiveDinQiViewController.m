...
#import "SecondTableViewCell.h"
#import "ProuctModel.h"
@interface FiveDinQiViewController (){
...
@implementation FiveDinQiViewController
...
-(void)getNetworkData:(BOOL)isRefresh
{
  ...
  //   [self showProgress];
    ...
    if (isRefresh) {
        url = [NSString stringWithFormat:@"%@?page=1&rows=20&userId=%@&productCategoryId=0&state=3",PRODUCTO_RDERS_URL,userID];
        
    }else{
        url = [NSString stringWithFormat:@"%@?page=%d&rows=20&userId=%@&productCategoryId=0&state=3",PRODUCTO_RDERS_URL,page,userID];
